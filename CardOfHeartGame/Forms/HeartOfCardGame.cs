...
{
    public class HeartOfCardGame : System.Windows.Forms.Form
    {
        #region View constants
        private const int ShowCardWidth = 20;
        private const int Player1X = 20;
        private const int Player1Y = 140;
        ...
        private const int StartButtonWidth = 125;
        private const int StartButtonHeigth = 50;
        #endregion

        private bool delayElapsed = false;
        private delegate int PlayerPlayDelegate(int player);
        private PlayerPlayDelegate[] PlayerPlay = new PlayerPlayDelegate[2];
        ...
        private void NewPeriod()
        {
            ...
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 12; j++)
                {
                    switch (i)
                    {
                        case 0: all[i * 12 + j].TopLeft = new Point(Player1X, Player1Y + j * ShowCardWidth); break;
            ...
            LowMixCards();
            ArrangeCards();
            for (int i = 0; i < 2; i++) remainCards[i].Clear();
            for (int cardInd = 0; cardInd < 24; cardInd++) remainCards[cardInd / 12].Add(cardInd);
        ...
        private void Play(int ind)
        {
            ...
            {
                case 0://player1
                    showWidthLeft = clearWidthLeft = Player1X;
                    showWidthRight = clearWidthRight = Player1X + Card.Width;
        ...
        private void GetRemainedCardsNum(int player, ref int[] num, ref int[] starts)
        {
            ...
            Debug.WriteLine("\tPlayer = " + player);
            for (int i = 0; i < 2; i++) num[i] = starts[i] = 0;
            int counter = 0;
            ArrayList myCards = remainCards[player];
            foreach (CardType ct in Enum.GetValues(typeof(CardType)))
            {
        ...
        private void DrawPlayerName(Graphics grp)
        {
            ...
            namesRect.Location = new Point(Player1X, Player1Y - 20);
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            {
                bool userShouldPlay = (centerCardNum + deckWinner % 4) == 0;
                Rectangle userCardsRect = new Rectangle(Player1X
                    , Player1Y
                    , Player1X + ShowCardWidth * 11 + Card.Width
                    , Player1Y + Card.Heigth);
                ...
                if (userCardsRect.Contains(e.X, e.Y) && deckNum != -1 && !startButtonVisible)
                {
                    ...
                    if (userShouldPlay)
                    {
                        int ind = (e.X - Player1X) / ShowCardWidth;
                        if (ind > 11) ind = 11;
                        ...
                        if (finalInd == -1)
                        {
                            ...
                            while (finalInd < remainCards[3].Count && (int)remainCards[3][finalInd] < ind + 36)
                                finalInd++;
                            finalInd = finalInd - 1;
                            if ((int)remainCards[3][finalInd] < 36 + ind - 3)
                                finalInd = -1;
                        }
                        if (finalInd != -1)
                        {
        ...
        private void DrawStartButton(Graphics g, Color c)
        {
            ...
            Rectangle startRect = new Rectangle(StartButtonX
                ...
                , StartButtonWidth
                , StartButtonHeigth);
            g.DrawString("Start Game", fnt, Brushes.White
                , startRect
                , sf);
        }
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(HeartOfCardGame));
            ...
            // 
            this.about.Index = 0;
            this.about.Text = "Rules && Settings";
            ...
            this.ClientSize = new System.Drawing.Size(581, 679);
